...
from archer.algorithms.test_loop import test_loop
from archer.algorithms.dpo_train_loop import dpo_train_loop
...
def main(config: "DictConfig"):
    ...
    if config.env_name == "twenty_questions":
        env = BatchedTwentyQuestionsEnv(env_load_path=config.env_load_path, 
                                        device=device, 
                                        cache_dir=config.cache_dir,bsize=config.batch_size)
        eval_env = BatchedTwentyQuestionsEnv(env_load_path=config.env_load_path,
                                             device=device,
                                                cache_dir=config.cache_dir,bsize=32)
    ...
    if config.agent_type.lower() == "chai":
        ...
        agent = ArcherAgent(device=device, accelerator=accelerator, 
                            ...
                            eos_str=config.eos_str,use_bfloat16=config.use_bfloat16)
        ...
        agent = ArcherAgent(device=device, accelerator=accelerator, 
                            temperature=config.temperature, do_sample=config.do_sample, 
                            policy_lm=config.policy_lm, critic_lm=config.critic_lm,
                            cache_dir=config.cache_dir, max_new_tokens=config.max_new_tokens,ref=True,
