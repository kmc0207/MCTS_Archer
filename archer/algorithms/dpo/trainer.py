...
class DPOTrainer():
    def __init__(self,
        ...
        dtype = next(self.agent.model.parameters()).dtype
        device = accelerator.device
        if use_ppo:
            if not hasattr(self.agent.model, "reward_head"):
                hidden_size = getattr(self.agent.model.config, "hidden_size", 768)
                self.agent.model.reward_head = nn.Linear(hidden_size, 1)
                
                # 모델의 dtype, 디바이스에 맞춰서 reward_head도 옮겨줌
                self.agent.model.reward_head.to(device=device, dtype=torch.bfloat16)
    ...
    def actor_reward_loss(
        ...
        **kwargs
        ...
        """
        - observation/action을 토크나이즈하고
        - 모델의 마지막 hidden state를 뽑은 후 reward_head에 통과
        - mc_return과 MSELoss 계산 후 backward
        - (중요) bfloat16 / float32 등에 맞춰 dtype을 통일
        """
        device = self.accelerator.device
        
        # 1) 텐서화 (dtype은 '모델과 같은 dtype'으로 통일)
        #    모델 파라미터를 하나 골라 dtype을 확인
        ...
        # 토큰을 모델 device + model_dtype으로
        for k, v in inputs.items():
            inputs[k] = v.to(device=device)
            # 보통 토큰 id/attention mask는 int형이므로, 여기서는 굳이 float으로 변환하지 않아도 됩니다.
            # 만약 embedding부터 bf16으로 처리해야 한다면, cast(int->bf16) 대신 autocast 등을 활용할 수도 있음
        
        # 3) 모델 forward + hidden states
        #    (마지막 hidden_state: shape [batch, seq_len, hidden_size])
        outputs = self.accelerator.unwrap_model(self.agent).model(
            **inputs,
            output_hidden_states=True
        )
        last_hidden_state = outputs.hidden_states[-1]  # (batch, seq_len, hidden_size)

        # 4) 마지막 토큰만 꺼내서 reward_head에 통과
        final_state = last_hidden_state[:, -1, :]
        
        # reward_head 자체가 이미 model_dtype일 텐데,
        # final_state가 fp32로 남아있으면 mismatch가 날 수 있으므로 캐스팅
        final_state = final_state.to(model_dtype)
        
        pred_reward = self.accelerator.unwrap_model(self.agent).model.reward_head(final_state)
        # (batch, 1) -> squeeze
        pred_reward = pred_reward.squeeze(-1)  # shape: (batch,)

        # 5) mc_return도 model_dtype으로 맞춤
        #    혹은 bf16 모드에서 float32 텐서가 들어오면 mismatch 에러
        if not isinstance(mc_return, torch.Tensor):
            mc_return = torch.tensor(mc_return, device=device, dtype=model_dtype)
        else:
            mc_return = mc_return.to(device=device, dtype=model_dtype)

        # 6) MSELoss
        loss = self.mse_loss_fn(pred_reward, mc_return)
        
        # 7) backward
        self.accelerator.backward(loss)

        return {
            "reward_loss": loss.detach().float().item()  # 로그 용으로 float 변환
        }
    ...
    def reward_update(self, replay_buffer, no_update_actor=False, reward_loss_weight=1.0):
        """
        reward buffer에 들어있는 mc_return을 이용해
        actor가 예측하는 값이 이를 맞추도록 regression loss를 계산하는 메서드.
        """
        self.step += 1
        info = {}
        info_list = []
        ...
        if not no_update_actor:
            ...
            for _ in range(self.epochs):
                ...
                # grad_accum_steps * batch_size 만큼 샘플링
                ...
                for batch in dataloader:
                    # batch 에는 observation, action, mc_return 등이 들어 있다고 가정
                    mc_return = batch.get("mc_return", None)
                    observation = batch.get("observation", None)
                    action = batch.get("action", None)
                    if mc_return is None or observation is None:
                        # 혹은 필요한 필드가 없으면 스킵
                        continue

                    # loss 계산
                    losses_dict = self.actor_reward_loss(
                        observation=observation,
                        action=action,
                        mc_return=mc_return
                    )
                    # 만약 가중치를 줄 때
                    total_loss = reward_loss_weight * torch.tensor(losses_dict["reward_loss"], 
                                                                   device=self.accelerator.device)
                    # 이미 backward를 한 번 했으므로 추가 곱셈 형태는 불필요할 수 있지만,
                    # "reward_loss"가 아직 텐서 형태가 아니라면 변환이 필요.
                    # 여기서는 단순 통계 계산용으로 info_list에만 남깁니다.
                    info_list.append({"reward_loss": float(total_loss.item())})
